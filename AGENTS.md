...
`bentway` is a published open-source library — five `@bentway/*` packages
implementing a runtime-agnostic execution kernel for LLM agent loops.
Treat every change as something that will appear on a stranger's
...
1. **Per-package gates** (`pnpm -r run test`)
   - `check-file-sizes.sh` — every source file under the per-package
     line cap (500 LOC default; turn-loop is allowlisted at 950).
   - `check-api-report.sh` — `@microsoft/api-extractor` diffs the live
     extraction of each package's public API against the committed
     `etc/*.api.md` snapshot. Drift = FAIL.

2. **Workspace gates** (root `pnpm test`)
   - `test:typecheck` — `tsc` against every test file under
     `tsconfig.typecheck-tests.json` (strict; zero errors required).
   - `test:hygiene` — `scripts/check-comment-hygiene.mjs`. Refuses the
     internal-process leakage patterns listed below.
   - `test:unit` — vitest projects, one per package, plus shared
     setup/harness in top-level `test/`.
...
- **Orphaned year-month dates** — `YYYY-MM` without a trailing `-DD`
  (full `YYYY-MM-DD` strings such as the Anthropic API version
  `2023-06-01` pass; the regex uses a negative lookahead)
...
- planning, history, or commit-message text in source
  ("originally we…", "before the X refactor…", "see Phase Y")
...
nothing host-specific"). That is the whole header. Examples of how to
write one live in the existing modules under `packages/*/src/`.
...
The `packages/*/etc/*.api.md` reports are the API every external
consumer programs against. Treat them as a checked-in contract:
...
3. `(cd packages/<pkg> && pnpm dlx @microsoft/api-extractor run --local)`
   regenerates `etc/<pkg>.api.md`. For `@bentway/core`, run the five
   per-subpath configs (`api-extractor.<sub>.json`).
...
bentway/
├── packages/
│   ├── stream-json/         src/index.mjs
│   ├── core/                src/{turn-loop,transcript,tool-exec}.mjs
│   │                        src/normalize/{stop-reason,retryable}.mjs
│   ├── anthropic/           src/{anthropic,index,pricing,
│   │                             serialize-request,thinking-config}.mjs
│   ├── openai/              src/{openai,index,pricing,
│   │                             model-config,serialize-request}.mjs
│   └── llama/               src/{llama,index}.mjs
│
│   Each package also holds:
│       api-extractor.json   public-API config
│       etc/*.api.md         committed API snapshot (the contract)
│       scripts/check-*.sh   per-package gates
│       test/                vitest specs (this package's surface)
│       tsconfig.json        self-contained, no extends
│
├── test/                    shared _harness.ts + _setup.ts (frozen clock)
├── scripts/
│   └── check-comment-hygiene.mjs   workspace-wide hygiene gate
├── tsconfig.typecheck-tests.json   strict tsc for test files only
├── vitest.config.ts                projects-mode, one entry per package
└── .github/workflows/
    ├── ci.yml                      build + gates + hygiene + typecheck + unit
    └── publish.yml                 tag-triggered OIDC publish + provenance
```

Every package compiles to `dist/*.d.mts` via `tsc` with
`allowJs + emitDeclarationOnly` over JSDoc-typed `.mjs` source.

## Commands

```bash
pnpm install         # restore + topo prepare (each package runs tsc)
pnpm run build       # explicit rebuild
pnpm run test:gates  # per-package: file-size + api-report
pnpm run test:hygiene  # workspace: comment-hygiene scan
pnpm run test:typecheck  # tsc against test files
pnpm run test:unit   # vitest projects, all packages
